  ...
  render() {
    return (
      <div>
        ...
        <p>Baut man nur eine Pflanzenart an, wird der Boden einseitig ausgelaugt und Schädlinge und Krankheiten können sich rasant verbreiten. Deshalb ist es zwingend ratsam unterschiedliche Gemüsearten zu kombinieren, so dass diese nebeneinander gedeihen und voneinander profitieren können. Diese harmonische Pflanzengemeinschaft bezeichnet man als Mischkultur.  Sie eignet sich besonders für kleine Gärten, in denen ein richtiger Fruchtwechsel zu aufwändig wäre. Für eine erfolgreiche Mischkultur ist es wichtig die Eigenschaften seiner Pflanzen zu kennen. <br />Als Hilfe dient folgende Tabelle: <span onClick={this.toggleTable.bind(this)} style={{fontWeight: 'bold', color: 'rgb(6, 97, 27)', cursor: 'pointer'}}>{this.state.tableVisible? 'ausblenden':'einblenden'}</span></p>
